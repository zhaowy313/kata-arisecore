...
export default class CanvasBoard extends BoardBase {
  ...
  resize() {
    ...
    const rightOffset =
      marginSize + (this.config.coordinates && !this.config.viewport.right ? 0.5 : 0);
    const bottomOffset =
      marginSize + (this.config.coordinates && !this.config.viewport.bottom ? 0.5 : 0);
    const leftOffset =
      marginSize + (this.config.coordinates && !this.config.viewport.left ? 0.5 : 0);
          ...
          this.objects.forEach((object) => {
            const handler =
              typeof object.type === 'string'
                ? this.config.theme.drawHandlers[object.type]
                : object.type;
    ...
    if (!Array.isArray(boardObject)) {
      if (typeof boardObject.type === 'string') {
        if (!this.config.theme.drawHandlers[boardObject.type]) {
          ...
          throw new TypeError(
            `Board object type "${boardObject.type}" doesn't exist in \`config.theme.drawHandlers\`.`,
          );
        }
      } else {
        throw new TypeError(
          'Invalid board object type. Custom canvas board objects not yet supported.',
        );
      }
    }
  ...
  on(type: string, callback: ({ point }: { point: Point | null }) => void) {
