...
export default class GridLayer extends CanvasLayer {
  ...
  drawGrid() {
    ...
    if (this.board.config.theme.starPoints[this.board.config.size]) {
      for (const key in this.board.config.theme.starPoints[this.board.config.size]) {
        this.context.beginPath();
        this.context.arc(
          this.board.getX(this.board.config.theme.starPoints[this.board.config.size][key].x),
          this.board.getY(this.board.config.theme.starPoints[this.board.config.size][key].y),
          params.starSize * this.board.fieldSize,
          0,
          2 * Math.PI,
          true,
        );
  ...
  drawCoordinates() {
    ...
    this.context.font = `${params.bold ? 'bold ' : ''}${this.board.fieldSize / 2}px ${
      this.board.config.theme.font || ''
    }`;
    ...
    const coordinatesX = this.board.config.theme.coordinates.labelsX;
    const coordinatesY = this.board.config.theme.coordinates.labelsY;
